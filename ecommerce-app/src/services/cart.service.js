...
export async function addItem(userId, productId, quantity) {
  ...
  if (!Number.isInteger(pid) || pid <= 0 || !Number.isInteger(qty) || qty < 1 || qty > 999) {
  ...
  if (product.stock < qty) return { type: 'validation_error', message: 'Insufficient stock' };
...
export async function removeItem(userId, productId) {
  const pid = parseInt(productId, 10);
  if (!Number.isInteger(pid) || pid <= 0) return { type: 'validation_error', message: 'Invalid product id' };
  const cartId = await getOrCreateCartId(userId);
...
export async function merge(userId, items) {
  ...
  for (const it of items) {
    ...
    if (!Number.isInteger(pid) || pid <= 0 || !Number.isInteger(qty) || qty < 1) continue;
    const product = await getProductStockAndPrice(pid);
    if (!product) continue;
