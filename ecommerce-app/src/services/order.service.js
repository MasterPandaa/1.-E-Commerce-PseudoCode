...
export async function placeOrder (userId, address, payment, idempotencyKey) {
  ...
  try {
    ...
    orderItems = rows.map((r) => {
      ...
      if (r.stock < r.quantity) {
        throw new Error(`Insufficient stock for product ${r.product_id}`)
      }
  ...
  if (paymentResp.status !== 'approved') {
    return {
      type: 'payment_error',
      message: paymentResp.reason || 'Payment declined'
    }
  }
  ...
  try {
    ...
    // Attempt refund
    try {
      await paymentProvider.refund(paymentResp.paymentId)
    } catch {}
    if (e.message === 'stock_conflict') {
      return {
        type: 'conflict_error',
        message: 'Stock changed; order not placed'
      }
    }
...
export async function getInvoice (orderId, currentUser) {
  ...
  if (!Number.isInteger(oid) || oid <= 0) {
    return { type: 'validation_error', message: 'Invalid order id' }
  }
  const owns =
    currentUser.role === 'admin'
      ? true
      : await userOwnsOrder(currentUser.id, oid)
