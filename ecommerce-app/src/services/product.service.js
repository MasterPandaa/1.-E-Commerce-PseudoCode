...
export async function create (data, imageFile, currentUser) {
  ...
  if (!name || name.length < 2 || name.length > 150) {
    return { type: 'validation_error', message: 'Invalid name' }
  }
  if (!description || description.length < 10) {
    return { type: 'validation_error', message: 'Invalid description' }
  }
  if (!Number.isFinite(price) || price <= 0) {
    return { type: 'validation_error', message: 'Invalid price' }
  }
  if (!Number.isInteger(categoryId) || categoryId <= 0) {
    return { type: 'validation_error', message: 'Invalid category' }
  }
  if (!Number.isInteger(stock) || stock < 0) {
    return { type: 'validation_error', message: 'Invalid stock' }
  }
  ...
  if (!isValidMagic) {
    return { type: 'validation_error', message: 'Invalid image file' }
  }
  ...
  const id = await createProductRecord({
    name,
    description,
    price,
    categoryId,
...
export async function list (query) {
  ...
  return {
    type: 'success',
    data: { ...data, page: filters.page, limit: filters.limit }
  }
}
...
export async function get (id) {
  ...
  if (!Number.isInteger(pid) || pid <= 0) {
    return { type: 'validation_error', message: 'Invalid ID' }
  }
...
export async function update (id, data, imageFile, currentUser) {
  ...
  if (data.categoryId != null) {
    ...
    const cat = await getCategoryById(categoryId)
    if (!cat) {
      return { type: 'validation_error', message: 'Category not found' }
    }
