...
export async function loginUser(email, password, deviceInfo, ip) {
  ...
  if (user.status !== 'active') return { type: 'auth_error', message: 'Account suspended' };
...
export async function requestPasswordReset(email, appBaseUrl) {
  ...
  return { type: 'success', message: 'If the email exists, a reset link has been sent' };
}
...
export async function verifyResetToken(rawToken) {
  ...
  return { type: 'success', data: { resetId: rec.id, userId: rec.user_id, email: rec.email } };
}
