...
export async function updateOrderStatus(req, res) {
  const result = await adminService.updateOrderStatus(req.user, req.params.id, req.body.status);
  ...
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to update order status' } });
}
...
export async function changeUserStatus(req, res) {
  const result = await adminService.changeUserStatusAdmin(req.user, req.params.id, req.body.status);
  if (result.type === 'success') return res.status(200).json({ message: result.message });
  if (result.type === 'auth_error') return res.status(403).json({ error: { message: result.message } });
  if (result.type === 'not_found') return res.status(404).json({ error: { message: result.message } });
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to change user status' } });
}
...
export async function lowStockProducts(req, res) {
  ...
  if (result.type === 'success') return res.status(200).json(result.data);
  if (result.type === 'auth_error') return res.status(403).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to fetch low-stock products' } });
}
