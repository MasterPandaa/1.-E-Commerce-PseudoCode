...
export async function requestPasswordReset (req, res) {
  ...
  return res
    .status(500)
    .json({ error: { message: 'Could not process reset request' } })
}
...
export async function verifyPasswordReset (req, res) {
  ...
  if (result.type === 'success') {
    return res.status(200).json({ valid: true, email: result.data.email })
  }
  return res
    .status(400)
    .json({ valid: false, error: { message: result.message } })
}
