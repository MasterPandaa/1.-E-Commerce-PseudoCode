...
export async function validateCheckout(req, res) {
  ...
  const result = await orderService.validateCheckout(req.user.id, address, payment, idempotencyKey);
...
export async function placeOrder(req, res) {
  const { address, payment, idempotencyKey } = req.body;
  const result = await orderService.placeOrder(req.user.id, address, payment, idempotencyKey);
  if (result.type === 'success') return res.status(201).json(result.data);
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  if (result.type === 'conflict_error') return res.status(409).json({ error: { message: result.message } });
  if (result.type === 'payment_error') return res.status(402).json({ error: { message: result.message } });
...
export async function getInvoice(req, res) {
  ...
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to generate invoice' } });
}
