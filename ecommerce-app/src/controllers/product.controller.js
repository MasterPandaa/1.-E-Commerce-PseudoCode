...
export async function createProduct(req, res) {
  ...
  if (result.type === 'auth_error') return res.status(403).json({ error: { message: result.message } });
...
export async function listProducts(req, res) {
  ...
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to fetch products' } });
}
...
export async function getProduct(req, res) {
  ...
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to fetch product' } });
}

// PSEUDO: Update Product (admin only)
export async function updateProduct(req, res) {
  const result = await productService.update(req.params.id, req.body, req.file, req.user);
  if (result.type === 'success') return res.status(200).json({ message: 'Updated' });
  if (result.type === 'auth_error') return res.status(403).json({ error: { message: result.message } });
  if (result.type === 'not_found') return res.status(404).json({ error: { message: result.message } });
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to update product' } });
}

// PSEUDO: Delete Product (admin only)
export async function deleteProduct(req, res) {
  const result = await productService.remove(req.params.id, req.user);
  if (result.type === 'success') return res.status(200).json({ message: 'Deleted' });
  if (result.type === 'auth_error') return res.status(403).json({ error: { message: result.message } });
  if (result.type === 'not_found') return res.status(404).json({ error: { message: result.message } });
  if (result.type === 'validation_error') return res.status(400).json({ error: { message: result.message } });
  return res.status(500).json({ error: { message: 'Failed to delete product' } });
}
