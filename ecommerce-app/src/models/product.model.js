...
export async function getCategoryById(categoryId) {
  const rows = await query('SELECT id FROM categories WHERE id = ? LIMIT 1', [categoryId]);
  return rows[0] || null;
}
...
export async function createProductRecord({ name, description, price, categoryId, stock, imageUrl }) {
...
export async function softDeleteProduct(id) {
  const res = await query('UPDATE products SET is_deleted = 1, updated_at = NOW() WHERE id = ? AND is_deleted = 0', [id]);
...
export async function findProducts(filters) {
  ...
  const items = await query(rowsQuery, [...params, filters.limit, (filters.page - 1) * filters.limit]);
  return { total, items };
}
