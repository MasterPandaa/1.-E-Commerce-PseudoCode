...
export async function getOrderWithItems(orderId) {
  ...
  const items = await query('SELECT product_id, unit_price, quantity FROM order_items WHERE order_id = ?', [orderId]);
  return { order, items };
}

export async function userOwnsOrder(userId, orderId) {
  const rows = await query('SELECT 1 FROM orders WHERE id = ? AND user_id = ? LIMIT 1', [orderId, userId]);
  return rows.length > 0;
}

export async function hasProcessedIdempotency(userId, key) {
  const rows = await query('SELECT order_id FROM idempotency_keys WHERE user_id = ? AND key_value = ? LIMIT 1', [userId, key]);
  return rows[0] || null;
}
...
export async function deductStock(conn, productId, quantity) {
  const [res] = await conn.execute('UPDATE products SET stock = stock - ? WHERE id = ? AND stock >= ?', [quantity, productId, quantity]);
  return res.affectedRows;
}

export async function createOrder(conn, userId, address, totals, paymentStatus, paymentId) {
...
export async function getSalesStats({ startDate, endDate, groupBy }) {
  ...
  const fmt = groupBy === 'week' ? '%x-%v' : groupBy === 'month' ? '%Y-%m' : '%Y-%m-%d';
...
export async function findOrdersAdmin({ page, limit, status, userId, startDate, endDate }) {
...
export async function updateOrderStatus(orderId, newStatus) {
  const res = await query('UPDATE orders SET status = ?, updated_at = NOW() WHERE id = ?', [newStatus, orderId]);
  return res.affectedRows;
}
